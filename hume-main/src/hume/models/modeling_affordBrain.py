...
# Pad Mask的作用是表示这段输入中哪些部分是真实值，哪些部分仅仅是填充的
# Att Mask的作用是表示这段输入中哪些部分可以相互注意，哪些部分不能注意
# 这个函数的作用是根据pad_masks和att_masks生成一个2D的注意力掩码，用于后续的注意力计算
...
# 这个类是完整的policy包装，其中包含了 System1 和 System2 的模型, 并且初始化了VQ-head
...
# 仅包含System 2的policy实现
# System2Policy只使用System2模型处理决策
# HumePolicy实现了完整的双系统架构(System1 + System2)
# System2Policy更简单，只包含model = System2(config)
# 没有FastVisuoMatching(S1)和ValueQueryHead组件
...
# 这个是System2的实现，是HumePolicy的一个组成部分
...
# 这个是System1的实现，是HumePolicy的一个组成部分
...
# 这个是ValueQueryHead的实现，是HumePolicy的一个组成部分
