...
class LeRobotDataset(BaseLeRobotDataset):
    def __init__(
        ...
        # 这里我们将我们的dataset的图像键重命名为observation.images.*，以便与libero一致
        rename_image_keys: bool = True,
    ):
        super().__init__(
            repo_id=repo_id,
            root=root,
        ...
        self.rename_image_keys = rename_image_keys
    ...
    def __getitem__(self, idx) -> dict:
        ...
        # 如果需要重命名图像键
        if self.rename_image_keys:
            for key in list(item.keys()):
                if key in ["image", "wrist_image"]:
                    item[f"observation.images.{key}"] = item[key]

                # If rename_image_keys is enabled, ensure s1_item and vqh_item have observation.images.* keys
        if self.rename_image_keys:
            for img_key in ["image", "wrist_image"]:
                if img_key in s1_item:
                    s1_item[f"observation.images.{img_key}"] = s1_item[img_key]
            for img_key in ["image", "wrist_image"]:
                if img_key in vqh_item:
                    vqh_item[f"observation.images.{img_key}"] = vqh_item[img_key]
        ...
        if self.image_transforms is not None:
            image_keys = list(filter(lambda key: "wrist" not in key, camera_keys))
            # 输出调试信息
            print("🔍 Camera keys:", camera_keys)
            print("🔍 Image keys:", image_keys)
            print("🔍 Item keys:", list(item.keys()))
            print("🔍 S1 Item keys:", list(s1_item.keys()))
            print("🔍 Meta camera keys:", self.meta.camera_keys)
            ...
            for cam in image_keys:
                if cam in item:
                    ...
                    print(f"⚠️ Warning: Key '{cam}' not found in item")

                # try:
                #     item[cam] = self.image_transforms(item[cam])
                # except KeyError:
                #     # 如果图像键不存在，输出当前item的keys和期望的图像keys
                #     print(f"❗ KeyError: '{cam}' not found in item. Available keys: {list(item.keys())}")
                #     raise

        if self.wrist_transforms is not None:
            wrist_keys = list(filter(lambda key: "wrist" in key, camera_keys))
            print("🔧 Wrist keys:", wrist_keys)
            for wrist in wrist_keys:
                if wrist in item:
                    item[wrist] = self.wrist_transforms(item[wrist])
                else:
                    print(f"⚠️ Warning: Key '{wrist}' not found in item")
